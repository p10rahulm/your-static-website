...
document.addEventListener('DOMContentLoaded', function(){
    let num_stars = {{.Params.stars}}
    console.log("stars = " + num_stars);
    let integral_stars = Math.floor(num_stars);
    console.log("integral_stars = " + integral_stars);




    var parent = document.getElementById("review_star_container");
    for(x = 0; integral_stars > x ; x++ ){
        var star = document.createElement("span");
        star.setAttribute("class","fa fa-star checked")
        parent.appendChild(star);
    }
    if( ! Number.isInteger(num_stars)){
        var half_star = document.createElement("span");
        half_star.setAttribute("class","fa fa-star-half-o checked")
        parent.appendChild(half_star);
    }
